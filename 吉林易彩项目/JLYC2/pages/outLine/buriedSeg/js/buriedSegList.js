 ...
 * 新增直埋段
 */
function addBuriedSeg() {
	mui.openWindow("buriedSegAdd.html");
}
/**
 * 
 * @param {Object} buriedSegName
 */
function getBuriedSegList(index) {
		...
		success: function(response) {
			...
			if(result == '0') {
				...
				for(var i = cells.length, len = i + infos.length; i < len; i++) {
					...
					$("#buriedSegUl").prepend("<div class=\"con borderbottom\" onclick=\"getBuriedSegDetail('" + info.int_id + "','" + info.zh_label + "')\";>" +
...
/**
 * 得到
 * @param {Object} buriedSegId
 * @param {Object} buriedSegName
 */
function getBuriedSegDetail(int_id, zh_label) {
	mui.openWindow({
...
function searchBuriedSeg() {
	var zh_label = $("#buriedSegName").val();
	plus.nativeUI.showWaiting("正在加载数据......");
	var UID = plus.storage.getItem("uid"); //获取缓存记录
	var logingUser = plus.storage.getItem("logingUser"); //当前登录用户
	var areaName = plus.storage.getItem("areaName"); //得到权限控制地市
