		...
		mui.ajax(url, {
			...
			success: function(response) {
				...
				if(result == '0') {
					...
					$("#start_PointName").val(info.start_PointName);
					$("#startLon").val(info.startLon);
					$("#startLat").val(info.startLat);
					$("#end_PointName").val(info.end_PointName);
...
function updateBuridSeg(){
	...
	var logingUser = plus.storage.getItem("logingUser");//当前登录用户
	var areaName = plus.storage.getItem("areaName");//得到权限控制地市
	var url = plus.storage.getItem("url")+"/pdaBuriedPartCustom!updateBuriedPartCustom.interface";
	mui.ajax(url, {
		type: 'post',
		dataType: 'json',
		timeout: 10000 * 60,
		...
		success: function(response) {
			...
			if(result =='0'){
				var btnArray = ['继续', '返回'];
				mui.confirm('直埋段修改成功，是否返回直埋段列表?','提示信息', btnArray, function(e) {
					if (e.index == 1) {
...
function deleteBuridSeg(){
	...
	mui.confirm('确定删除吗?', '提示信息', btnArray, function(e) {
		if (e.index == 0) {
			...
			var logingUser = plus.storage.getItem("logingUser");//当前登录用户
			var areaName = plus.storage.getItem("areaName");//得到权限控制地市
			var url = plus.storage.getItem("url") + "/pdaBuriedPartCustom!deleteBuriedPartCustom.interface";
			mui.ajax(url, {
				type: 'post',
				dataType: 'json',
				timeout: 10000 * 60,
