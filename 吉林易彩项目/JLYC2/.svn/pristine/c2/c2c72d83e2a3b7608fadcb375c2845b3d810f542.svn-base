			...
			if(result == '0') {
				...
				for(var i = cells.length,len = i + infos.length; i < len; i++) {
					...
					var resStr = getNewline(info.zh_label);
						$("#stoneUl").prepend("<div class=\"con borderbottom\" onclick=\"checkStone('"+info.int_id+"','"+info.zh_label+"','"+info.longitude+"','"+info.latitude+"')\";>"
 ...
 */
function checkStone(int_id,zh_label,lon,lat){
	var poleLineSeg = plus.webview.currentWebview().opener();//获取打开的页面信息
	if(resType == 'segAdd'){
		mui.fire(poleLineSeg, "buriedSegAdd", {int_id:int_id,zh_label:zh_label,operate:operate,lon:lon,lat:lat});
	}
	if(resType == 'segDetail'){
		mui.fire(poleLineSeg, "buriedSegDetail", {int_id:int_id,zh_label:zh_label,operate:operate,lon:lon,lat:lat});
	}
	mui.back();
}
