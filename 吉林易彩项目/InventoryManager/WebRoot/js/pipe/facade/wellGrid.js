...
var limit = 10;
	...
	{header : '操作',dataIndex : 'wellId',width : 100,
		renderer : function(value, metadata, record, rowIndex, colIndex, store) {
			return '<input type="button" value="选择此井" onclick="selectWell('+rowIndex+')" />';				
		}
	}]);
	// 从远程读取数据
...
com.increase.cen.poleline.WellGrid = Ext.extend(Ext.grid.GridPanel, {
	id:'wellGrid',
	cm : cm,// 列定义的model(必需)
	store : store,
	viewConfig : {
		...
		handler:function(){
			...
			store.load({
				params : {
					start : 0,
					limit : limit,
					'wellInfoBean.wellNameSub' : Ext.getCmp("wellNameSub").getValue(),
					'wellInfoBean.manholeTypeEnumId' : Ext.getCmp("manholeTypeEnumId").getValue(),
					'wellInfoBean.wellAddress' : Ext.getCmp("wellAddress").getValue()
				}
			});
			...
			Ext.getCmp("wellGrid").getView().refresh();
		}
	}, {
		xtype : 'tbseparator'
	}, {
	...
	initComponent : function(config) {
		...
		com.increase.cen.poleline.WellGrid.superclass.initComponent.call(this);
	}
});

Ext.reg("wellGrid", com.increase.cen.poleline.WellGrid);
