Ext.namespace("com.increase.cen.pipe");//tengxy
// 一页显示多少条数据
var limit = null;
com.increase.cen.pipe.importLedup = Ext.extend(Ext.form.FormPanel, {
	...
	initComponent : function(config) {
		...
		this.one = {
			...
			style : '',
			html : '<img border="0" src="imgs/plwh/s.gif" width="28" height="21"> 第一步 <a target="_self" href="export.jsp?filename=iLeduptmpl">下载模板</a>并按模板格式整理数据。',
		...
		com.increase.cen.pipe.importLedup.superclass.initComponent.call(this);
	},
	// 保存按钮事件
	btnSubmit : function() {
		// 提交验证
...
//					parent.parent.Ext.Msg.wait("服务器正在处理档案……", '请稍候'); 
		this.getForm().doAction("submit", {
			url : 'dataupdate!insertLedup.action',
			method : 'post',
			timeout: 1000*60*5,
			...
			success : function(form, action) {
				...
				Ext.getCmp('importLedupWindow').close();// 窗口隐藏
				Ext.getCmp('LedupGrid').getStore().reload();
				parent.Ext.Msg.show({
					title : '提示',
					width : 300,
...
Ext.reg("importLedup", com.increase.cen.pipe.importLedup);
