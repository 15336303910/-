...
var limit = 10;
	...
	{header : '操作',dataIndex : 'wellId',width : 100,
		renderer : function(value, metadata, record, rowIndex, colIndex, store) {
			return '<input type="button" value="选择此井" onclick="selectWell_a('+rowIndex+')" />';				
		}
	}]);
	// 从远程读取数据
...
com.increase.cen.poleline.WellGrid_A = Ext.extend(Ext.grid.GridPanel, {
	id:'wellGrid_a',
	cm : cm,// 列定义的model(必需)
	store : store,
	viewConfig : {
	...
	tbar : [{
		...
		handler:function(){
			...
			Ext.getCmp("wellGrid_a").getView().refresh();
		}
	}, {
		xtype : 'tbseparator'
	}, {
	...
	initComponent : function(config) {
		...
		com.increase.cen.poleline.WellGrid_A.superclass.initComponent.call(this);
	}
});

Ext.reg("wellGrid_a", com.increase.cen.poleline.WellGrid_A);
