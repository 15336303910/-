...
var limit = 10;
	...
	{header : '操作',dataIndex : 'poleId',width : 100,
		renderer : function(value, metadata, record, rowIndex, colIndex, store) {
			return '<input type="button" value="选择此电杆" onclick="selectPole_as('+rowIndex+')" />';				
		}
	}]);
	// 从远程读取数据
...
com.increase.cen.poleline.PoleGrid_As = Ext.extend(Ext.grid.GridPanel, {
	id:'poleGrid_as',
	cm : cm,// 列定义的model(必需)
	store : store,
	viewConfig : {
	...
	tbar : [{
		...
		handler:function(){
			...
			Ext.getCmp("poleGrid_as").getView().refresh();
		}
	}, {
		xtype : 'tbseparator'
	}, {
	...
	initComponent : function(config) {
		...
		com.increase.cen.poleline.PoleGrid_As.superclass.initComponent.call(this);
	}
});

Ext.reg("poleGrid_as", com.increase.cen.poleline.PoleGrid_As);
