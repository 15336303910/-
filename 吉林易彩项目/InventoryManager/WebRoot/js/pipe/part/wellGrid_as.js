...
var limit = 10;
	...
	{header : '操作',dataIndex : 'wellId',width : 100,
		renderer : function(value, metadata, record, rowIndex, colIndex, store) {
			return '<input type="button" value="选择此井" onclick="selectWell_as('+rowIndex+')" />';				
		}
	}]);
	// 从远程读取数据
...
com.increase.cen.poleline.WellGrid_As = Ext.extend(Ext.grid.GridPanel, {
	id:'wellGrid_as',
	cm : cm,// 列定义的model(必需)
	store : store,
	viewConfig : {
	...
	tbar : [{
		...
		handler:function(){
			...
			Ext.getCmp("wellGrid_as").getView().refresh();
		}
	}, {
		xtype : 'tbseparator'
	}, {
	...
	initComponent : function(config) {
		...
		com.increase.cen.poleline.WellGrid_As.superclass.initComponent.call(this);
	}
});

Ext.reg("wellGrid_as", com.increase.cen.poleline.WellGrid_As);
