...
var limit = 10;
	...
	{header : '操作',dataIndex : 'wellId',width : 100,
		renderer : function(value, metadata, record, rowIndex, colIndex, store) {
			return '<input type="button" value="选择此井" onclick="selectWell_zz('+rowIndex+')" />';				
		}
	}]);
	// 从远程读取数据
...
com.increase.cen.poleline.WellGrid_ZZ = Ext.extend(Ext.grid.GridPanel, {
	id:'wellGrid_zz',
	cm : cm,// 列定义的model(必需)
	store : store,
	viewConfig : {
	...
	tbar : [{
		...
		handler:function(){
			...
			store.load({
				params : {
					start : 0,
					limit : limit,
					'wellInfoBean.wellNameSub' : Ext.getCmp("wellNameSub_zz").getValue(),
					'wellInfoBean.manholeTypeEnumId' : Ext.getCmp("manholeTypeEnumId_zz").getValue(),
					'wellInfoBean.wellAddress' : Ext.getCmp("wellAddress_zz").getValue()
				}
			});
			...
			Ext.getCmp("wellGrid_zz").getView().refresh();
		}
	}, {
		xtype : 'tbseparator'
	}, {
		...
		cls : "x-btn-text-icon",
		handler:function(){
			Ext.getCmp("wellNameSub_zz").setValue("");
			Ext.getCmp("manholeTypeEnumId_zz").setValue("");
			Ext.getCmp("wellAddress_zz").setValue("");
			var store = Ext.getCmp("wellGrid_zz").getStore();
			store.load({
				params : {
					start : 0,
	...
	initComponent : function(config) {
		...
		com.increase.cen.poleline.WellGrid_ZZ.superclass.initComponent.call(this);
	}
});

Ext.reg("wellGrid_zz", com.increase.cen.poleline.WellGrid_ZZ);
