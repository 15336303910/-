Ext.onReady(function() {
   ...
   {header : "序号",width : 50,hideable : false,
	...
	refreshBtn.on('click', function() {
		Ext.getCmp("equtGrid").getStore().load({
			params : {
				'equt.etype':'1',
   					start : 0,
					limit : limit
...
//	var equtGrid=Ext.getCmp("equtGrid");
//	equtGrid.on('rowdblclick',function(g, r, e){
//		e.stopEvent();
...
//			}
//				var equtGrid=Ext.getCmp("equtGrid");
//				if(!equtGrid.seeEventWindow){
//					equtGrid.seeEventWindow=new com.increase.cen.log.logWindow({
//					id : "seeEventWindow",
//					height : 400,
//					width : 1000,
//					title : '查看日志',
	...
	seeEqut.on('click',function(){
		...
		if (selModel.hasSelection()) {
			...
			if (ids.length > 1) {
				...
				var equtGrid=Ext.getCmp("equtGrid");
				if(!equtGrid.seeEqutWindow){
					equtGrid.seeEqutWindow=new com.increase.cen.log.logWindow({
					id : "seeEqutWindow",
					height : 400,
					width : 1000,
					title : '查看资源日志',
					...
					items : [({
						height : 350,
						xtype : 'aloneEqut'// 窗口里的表单
					})]
				});
				}
				var store = Ext.getCmp("aloneEqut").getStore();
				var loadMarsk = new Ext.LoadMask(document.body, {
					msg : '数据加载中，请稍候......',
					disabled : false,
					store : store
