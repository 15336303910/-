...
//一页显示多少条数据
var limit = null;
com.increase.cen.poleline.importSupport = Ext.extend(Ext.form.FormPanel, {
	...
	initComponent : function(config) {
		...
		this.one = {
				...
				style : '',
				html : '<img border="0" src="imgs/plwh/s.gif" width="28" height="21"> 第一步 <a target="_self" href="export.jsp?filename=support">下载模板</a>并按模板格式整理数据。',
		...
		com.increase.cen.poleline.importSupport.superclass.initComponent.call(this);
	...
	btnSubmit : function() {
		...
		this.getForm().doAction("submit", {
			url : 'dataupdate!insertsupport.action',
			method : 'post',
			timeout: 1000*60*5,
			...
			success : function(form, action) {
				...
				Ext.getCmp("importsupport").getForm().reset();// 重置表单
				Ext.getCmp('importWindow').close();// 窗口隐藏
				Ext.getCmp('supportGrid').getStore().load({
					params : {
						start : 0,
						limit : limit
					}
				});
				Ext.getCmp('supportGrid').getView().refresh();
...
Ext.reg("importsupport", com.increase.cen.poleline.importSupport);
