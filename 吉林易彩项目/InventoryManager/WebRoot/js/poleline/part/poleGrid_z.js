...
var limit = 10;
	...
	{header : '操作',dataIndex : 'poleId',width : 100,
		renderer : function(value, metadata, record, rowIndex, colIndex, store) {
			return '<input type="button" value="选择此电杆" onclick="selectPole_z('+rowIndex+')" />';				
		}
	}]);
	// 从远程读取数据
...
com.increase.cen.poleline.PoleGrid_Z = Ext.extend(Ext.grid.GridPanel, {
	id:'poleGrid_z',
	cm : cm,// 列定义的model(必需)
	store : store,
	viewConfig : {
	...
	tbar : [{
		...
		handler:function(){
			...
			Ext.getCmp("poleGrid_z").getView().refresh();
		}
	}, {
		xtype : 'tbseparator'
	}, {
	...
	initComponent : function(config) {
		...
		com.increase.cen.poleline.PoleGrid_Z.superclass.initComponent.call(this);
	}
});

Ext.reg("poleGrid_z", com.increase.cen.poleline.PoleGrid_Z);
