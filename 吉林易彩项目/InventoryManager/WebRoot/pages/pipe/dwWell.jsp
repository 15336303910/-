...
String wellname = URLDecoder.decode((String)request.getParameter("wellname"),"utf-8");
 %>
...
<script type="text/javascript" src="http://api.map.baidu.com/api?key=&v=1.3&services=true" onerror="loadError();">
</script>
...
<body>
  ...
  <input id="wellname" type="hidden" value=<%=wellname %> />
</body>
<script type="text/javascript">
    //创建和初始化地图函数：
    function initMap(){
    ...
    wellname=wellname.replace("P","#");
    var point = lon+"|"+lat;
    var contentStr = "<table>"+
						"<tr><td>井地址：</td><td>"+addr+"</td></tr>"+
						"<tr><td>井名称：</td><td>"+wellname+"</td></tr>"+
						"</table>"
    var markerArr = [{title:addr,content:contentStr,point:point,isOpen:0,icon:{w:21,h:21,l:0,t:0,x:6,lb:5}}
		 ];
    //创建marker
    function addMarker(){
