...
public class TextUtil {
	 ...
	 * @return 处理后的字符串
	 */
	public static String encodeChinese(String chinese) {
		try {
			return java.net.URLEncoder.encode(chinese,"UTF-8");
	...
	public static String returnToBr(String sStr) {
		...
		/*暂时屏蔽,无法判断是否为手工输入的<br> 
		//清理原有<br>信息
		sStr=sStr.replaceAll("<br>","");
		*/
		sStr=ecodeHtmlToText(sStr);
	...
	/**
	 * 取得将给定字符串中的<全部转换成"&lt;"并>全部转换"&gt;"
	 * 
	 * @param sStr
	 *            字符串参数
	 * @return String 经过处理后的字符串.
	 */
	public static String ecodeHtmlToText(String sStr) {
		if (isNull(sStr)) {
			return sStr;
		}
		return sStr.replaceAll("<","&lt;").replaceAll(">","&gt;");
	}
	
	/**
	 * 以id为基准删除用splits分隔的字符串中相同的元素及空字符串元素
	 * @param oldIds 原id字符串
	 * @param splits 字符串元素之间的分隔符,可为null默认为","
	 * @param oldValues 原value字符串
	 *
	 * @return Map {[key=idStr为新的id字符串;key=valueStr为新的value字符串]
	 * 如果oldIds="1,2,3,1,2,3"与oldValues="11,12,13"的数值不对应则只返回处理后的id字符串}
	 */
	@SuppressWarnings("unchecked")
	public static Map  compressStringById(String oldIds,String splits,String oldValues){
		if(isNull(oldIds)){
			return null;
		}
		if(isNull(splits)){
		...
		StringBuffer tempNewId=new StringBuffer(splits);
		String[] oldIdsArray=oldIds.split(splits);
		String[] oldValuesArray={};
		if(isNotNull(oldValues)){
			oldValuesArray=oldValues.split(splits);
		}
		for(int i=0;i<oldIdsArray.length;i++){
			if(isNull(oldIdsArray[i])){
				continue;
			}
			String tempId=oldIdsArray[i];
			if(tempNewId.indexOf(splits+tempId+splits)<0){
				//如果为第一个元素,则直接加入新字符串
				if(isNull(newIds.toString())){
					newIds.append(tempId);
					if(i<oldValuesArray.length){
						newValues.append(oldValuesArray[i]);
					}
				}else{
					//如果不为第一个元素加入原有的分隔符
					newIds.append(splits).append(tempId);
					if(i<oldValuesArray.length){
						newValues.append(splits).append(oldValuesArray[i]);
					}
				}
				tempNewId.append(tempId).append(splits);
			}
		}
		strMap.put("idStr",newIds.toString());
		//如果value与id的数值对应则压入新的values
		if(isNotNull(oldValues)&&oldIdsArray.length==oldValuesArray.length){
			strMap.put("valueStr",newValues.toString());
		}
		return strMap;
	}
}
