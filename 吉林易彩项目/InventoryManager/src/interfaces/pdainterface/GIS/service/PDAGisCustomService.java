...
public class PDAGisCustomService extends DataBase implements IPDAGisCustomService{
	
	private JdbcTemplate jdbcTemplate;
	private static String siteSTR = "pdagenerator.getSiteBase";
	private static String equtSTR = "pdaequt.getEqutJiLin";
	...
	public List<GisPojo> getGisLine(GisPojo pojo){
		...
		if(TextUtil.isNotNull(wellList)){
			...
			for(int i=0;i<wellList.size();i++){
				...
				spotMap.put(order.get("id")+"",order.get("name")+"%_%"+order.get("longitude")+"%_%"+order.get("latitude"));
			}
	...
	String getSpotStr(GisPojo pojo){
		...
		if(pojo.getType().equals("pipe")){
			if(TextUtil.isNotNull(pojo.getName())){
				sql=""
					+ "select t.int_id as id,t.zh_label as name,"
					+ " t.longitude as longitude,t.latitude as latitude"
					+ " from rms_rsj t"
					+ " where t.stateflag ='0'"
					+ " and t.zh_label like '%"+pojo.getName()+"%'"
					+ " limit 100"
					+ "";
			}else{
				 sql=""
		...
		if(pojo.getType().equals("poleLine")){
			if(TextUtil.isNotNull(pojo.getName())){
				sql =""
					+ " SELECT t.int_id as id ,t.zh_label as name, "
					+ " t.longitude as longitude, t.latitude as latitude"
					+ " FROM rms_pole t where t.stateflag ='0'"
					+ " and t.zh_label like '%"+pojo.getName()+"%'"
					+ " limit 100"
					+ "";
			}else{
	...
	String getLineStr(String id,String type){
		String sql = "";
		if(type.equals("pipe")){
			sql ="select int_id as lineId,"
				+ " zh_label as lineName,"
				+ " dest_point_name as startId,orig_point_name as endId"
				+ " from rms_pipe_seg where stateflag ='0' and dest_point_name in ("
				+ ""+id+""
				+ ") and  orig_point_name in ("
				+ ""+id+" "
				+ ")";
		}
		if(type.equals("poleLine")){
			sql = " select int_id as lineId,zh_label as lineName,"
				+ " dest_point_name as startId,orig_point_name as endId from rms_pipe_seg"
				+ " where stateflag ='0' and dest_point_name in ("
				+ ""+id+""
				+ ") and  orig_point_name in ("
				+ ""+id+""
				+ ")";
		...
		if(type.equals("buried")){
			sql=" select  int_id as lineId,zh_label as lineName,"
				+ " start_ponit_id as startId,end_ponit_id as endId"
				+ " from rms_buried_seg where stateflag ='0' and start_ponit_id in ("
				+ ""+id+""
				+ ") and  end_ponit_id in ("
				+ ""+id+""
				+ ")";
