...
var limit = 10;
	...
	{header : '操作',dataIndex : 'wellId',width : 100,
		renderer : function(value, metadata, record, rowIndex, colIndex, store) {
			return '<input type="button" value="选择此井" onclick="selectWell_aa('+rowIndex+')" />';				
		}
	}]);
	// 从远程读取数据
	var proxy = new Ext.data.HttpProxy({
		url : "pipe!searchJSON.action"
...
com.increase.cen.poleline.WellGrid_AA = Ext.extend(Ext.grid.GridPanel, {
	id:'wellGrid_aa',
	cm : cm,// 列定义的model(必需)
	store : store,
	viewConfig : {
	...
	tbar : [{
		...
		handler:function(){
			...
			store.load({
				params : {
					start : 0,
					limit : limit,
					'wellInfoBean.wellNameSub' : Ext.getCmp("wellNameSub_aa").getValue(),
					'wellInfoBean.manholeTypeEnumId' : Ext.getCmp("manholeTypeEnumId_aa").getValue(),
					'wellInfoBean.wellAddress' : Ext.getCmp("wellAddress_aa").getValue()
				}
			});
			...
			Ext.getCmp("wellGrid_aa").getView().refresh();
		}
	}, {
		xtype : 'tbseparator'
	}, {
		...
		cls : "x-btn-text-icon",
		handler:function(){
			Ext.getCmp("wellNameSub_aa").setValue("");
			Ext.getCmp("manholeTypeEnumId_aa").setValue("");
			Ext.getCmp("wellAddress_aa").setValue("");
			var store = Ext.getCmp("wellGrid_aa").getStore();
			store.load({
				params : {
					start : 0,
	...
	initComponent : function(config) {
		...
		com.increase.cen.poleline.WellGrid_AA.superclass.initComponent.call(this);
	}
});

Ext.reg("wellGrid_aa", com.increase.cen.poleline.WellGrid_AA);
