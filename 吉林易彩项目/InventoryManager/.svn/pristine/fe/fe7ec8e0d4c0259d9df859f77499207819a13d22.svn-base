...
var limit = 10;
	...
	{header : '操作',dataIndex : 'poleId',width : 100,
		renderer : function(value, metadata, record, rowIndex, colIndex, store) {
			return '<input type="button" value="选择此电杆" onclick="selectPole_a('+rowIndex+')" />';				
		}
	}]);
	// 从远程读取数据
...
com.increase.cen.poleline.PoleGrid_A = Ext.extend(Ext.grid.GridPanel, {
	id:'poleGrid_a',
	cm : cm,// 列定义的model(必需)
	store : store,
	viewConfig : {
	...
	tbar : [{
		...
		handler:function(){
			...
			store.load({
				params : {
					start : 0,
					limit : limit,
					'poleInfoBean.poleNameSub':Ext.getCmp("poleNameSub").getValue(),
					'poleInfoBean.poleCode':Ext.getCmp("poleCode").getValue(),
					'poleInfoBean.poleAddress':Ext.getCmp("poleAddress").getValue()
				}
			});
			...
			Ext.getCmp("poleGrid_a").getView().refresh();
		}
	}, {
		xtype : 'tbseparator'
	}, {
		...
		cls : "x-btn-text-icon",
		handler:function(){
			Ext.getCmp("poleNameSub").setValue("");
			Ext.getCmp("poleCode").setValue("");
			Ext.getCmp("poleAddress").setValue("");
			var store = Ext.getCmp("poleGrid_a").getStore();
			store.load({
				params : {
					start : 0,
	...
	initComponent : function(config) {
		...
		com.increase.cen.poleline.PoleGrid_A.superclass.initComponent.call(this);
	}
});

Ext.reg("poleGrid_a", com.increase.cen.poleline.PoleGrid_A);
