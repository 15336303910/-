...
// 一页显示多少条数据
var limit = null;
com.increase.cen.pipe.importPipe = Ext.extend(Ext.form.FormPanel, {
	...
	initComponent : function(config) {
		...
		this.one = {
			...
			style : '',
			html : '<img border="0" src="imgs/plwh/s.gif" width="28" height="21"> 第一步 <a target="_self" href="export.jsp?filename=pipetmpl">下载模板</a>并按模板格式整理数据。',
		...
		com.increase.cen.pipe.importPipe.superclass.initComponent.call(this);
	},
	// 保存按钮事件
	btnSubmit : function() {
		// 提交验证
		...
		this.getForm().doAction("submit", {
			url : 'dataupdate!insertpipe.action',
			method : 'post',
			timeout: 1000*60*5,
			...
			success : function(form, action) {
				...
				Ext.getCmp('importPipeWindow').close();// 窗口隐藏
...
Ext.reg("importPipe", com.increase.cen.pipe.importPipe);
