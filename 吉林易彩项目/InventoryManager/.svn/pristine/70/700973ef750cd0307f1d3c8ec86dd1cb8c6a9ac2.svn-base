...
var limit = 10;
	...
	{header : '操作',dataIndex : 'wellId',width : 100,
		renderer : function(value, metadata, record, rowIndex, colIndex, store) {
			return '<input type="button" value="选择此井" onclick="selectWell_zs('+rowIndex+')" />';				
		}
	}]);
	// 从远程读取数据
...
com.increase.cen.poleline.WellGrid_Zs = Ext.extend(Ext.grid.GridPanel, {
	id:'wellGrid_zs',
	cm : cm,// 列定义的model(必需)
	store : store,
	viewConfig : {
	...
	tbar : [{
		...
		handler:function(){
			...
			store.load({
				params : {
					start : 0,
					limit : limit,
					'wellInfoBean.wellNameSub' : Ext.getCmp("wellNameSub_zs").getValue(),
					'wellInfoBean.manholeTypeEnumId' : Ext.getCmp("manholeTypeEnumId_zs").getValue(),
					'wellInfoBean.wellAddress' : Ext.getCmp("wellAddress_zs").getValue()
				}
			});
			...
			Ext.getCmp("wellGrid_zs").getView().refresh();
		}
	}, {
		xtype : 'tbseparator'
	}, {
		...
		cls : "x-btn-text-icon",
		handler:function(){
			Ext.getCmp("wellNameSub_zs").setValue("");
			Ext.getCmp("manholeTypeEnumId_zs").setValue("");
			Ext.getCmp("wellAddress_zs").setValue("");
			var store = Ext.getCmp("wellGrid_zs").getStore();
			store.load({
				params : {
					start : 0,
	...
	initComponent : function(config) {
		...
		com.increase.cen.poleline.WellGrid_Zs.superclass.initComponent.call(this);
	}
});

Ext.reg("wellGrid_zs", com.increase.cen.poleline.WellGrid_Zs);
