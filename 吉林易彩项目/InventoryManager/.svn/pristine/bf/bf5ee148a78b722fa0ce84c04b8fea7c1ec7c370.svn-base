Ext.onReady(function() {
	...
	var seeHFS=Ext.getCmp("seeHFS");
	seeHFS.on('click',function(){
		var selModel = grid.getSelectionModel();// 返回grid的SelectionModel
		if (selModel.hasSelection()) {
			var records = selModel.getSelections();
			var ids = [];
			for (var i = 0; i < records.length; i++) {
				ids.push(records[i].get("switchingId"));// 获得选择的编号
			}
			if (ids.length > 1) {
				Ext.Msg.show({
					title : '提示',
				...
				if(!HFSGrid.seeHFSWindow){
					HFSGrid.seeHFSWindow=new com.increase.cen.log.logWindow({
					id : "seeHFSWindow",
					height : 400,
					width : 1000,
					title : '查看资源日志',
					...
					items : [({
						height : 350,
						xtype : 'aloneHFS'// 窗口里的表单
					})]
				});
				}
				var store = Ext.getCmp("aloneHFS").getStore();
				var loadMarsk = new Ext.LoadMask(document.body, {
					msg : '数据加载中，请稍候......',
					disabled : false,
					store : store
