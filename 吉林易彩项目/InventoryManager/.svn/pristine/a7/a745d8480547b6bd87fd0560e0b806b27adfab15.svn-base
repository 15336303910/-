...
var limit = 10;
	...
	{header : '电杆编码',dataIndex : 'poleCode',width : 100,sortable : true}, 
	...
	{header : '电杆地址',dataIndex : 'poleAddress',width : 140,sortable : true}, 
	{header : '最后更新时间',dataIndex : 'lastUpdateDate',width : 130,sortable : true,
		renderer : Ext.util.Format.dateRenderer('Y-m-d H:i:s')
	},
	{header : '操作',dataIndex : 'poleId',width : 100,
		renderer : function(value, metadata, record, rowIndex, colIndex, store) {
			return '<input type="button" value="选择此电杆" onclick="selectPole_zz('+rowIndex+')" />';				
		}
	}]);
	// 从远程读取数据
...
com.increase.cen.poleline.PoleGrid_ZZ = Ext.extend(Ext.grid.GridPanel, {
	id:'poleGrid_zz',
	cm : cm,// 列定义的model(必需)
	store : store,
	viewConfig : {
	...
	tbar : [{
		...
		handler:function(){
			...
			store.load({
				params : {
					start : 0,
					limit : limit,
					'poleInfoBean.poleNameSub':Ext.getCmp("poleNameSub_z").getValue(),
					'poleInfoBean.poleCode':Ext.getCmp("poleCode_z").getValue(),
					'poleInfoBean.poleAddress':Ext.getCmp("poleAddress_z").getValue()
				}
			});
			...
			Ext.getCmp("poleGrid_zz").getView().refresh();
		}
	}, {
		xtype : 'tbseparator'
	}, {
		...
		cls : "x-btn-text-icon",
		handler:function(){
			Ext.getCmp("poleNameSub_z").setValue("");
			Ext.getCmp("poleCode_z").setValue("");
			Ext.getCmp("poleAddress_z").setValue("");
			var store = Ext.getCmp("poleGrid_zz").getStore();
			store.load({
				params : {
					start : 0,
	...
	initComponent : function(config) {
		...
		com.increase.cen.poleline.PoleGrid_ZZ.superclass.initComponent.call(this);
	}
});

Ext.reg("poleGrid_zz", com.increase.cen.poleline.PoleGrid_ZZ);
