...
var limit = 10;
	...
	{header : '操作',dataIndex : 'poleId',width : 100,
		renderer : function(value, metadata, record, rowIndex, colIndex, store) {
			return '<input type="button" value="选择此电杆" onclick="selectPole_zs('+rowIndex+')" />';				
		}
	}]);
	// 从远程读取数据
...
com.increase.cen.poleline.PoleGrid_Zs = Ext.extend(Ext.grid.GridPanel, {
	id:'poleGrid_zs',
	cm : cm,// 列定义的model(必需)
	store : store,
	viewConfig : {
	...
	tbar : [{
		...
		handler:function(){
			...
			store.load({
				params : {
					start : 0,
					limit : limit,
					'poleInfoBean.poleNameSub':Ext.getCmp("poleNameSub_zs").getValue(),
					'poleInfoBean.poleCode':Ext.getCmp("poleCode_zs").getValue(),
					'poleInfoBean.poleAddress':Ext.getCmp("poleAddress_zs").getValue()
				}
			});
			...
			Ext.getCmp("poleGrid_zs").getView().refresh();
		}
	}, {
		xtype : 'tbseparator'
	}, {
		...
		cls : "x-btn-text-icon",
		handler:function(){
			Ext.getCmp("poleNameSub_zs").setValue("");
			Ext.getCmp("poleCode_zs").setValue("");
			Ext.getCmp("poleAddress_zs").setValue("");
			var store = Ext.getCmp("poleGrid_zs").getStore();
			store.load({
				params : {
					start : 0,
	...
	initComponent : function(config) {
		...
		com.increase.cen.poleline.PoleGrid_Zs.superclass.initComponent.call(this);
	}
});

Ext.reg("poleGrid_zs", com.increase.cen.poleline.PoleGrid_Zs);
