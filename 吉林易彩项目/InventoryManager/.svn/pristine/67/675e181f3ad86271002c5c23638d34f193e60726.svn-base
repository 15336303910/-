...
com.increase.cen.poleline.PoleGrid_ZZ = Ext.extend(Ext.grid.GridPanel, {
	...
	tbar : [{
		...
		handler:function(){
			...
			store.load({
				params : {
					start : 0,
					limit : limit,
					'poleInfoBean.poleNameSub':Ext.getCmp("poleNameSub_zz").getValue(),
					'poleInfoBean.poleCode':Ext.getCmp("poleCode_zz").getValue(),
					'poleInfoBean.poleAddress':Ext.getCmp("poleAddress_zz").getValue()
				}
			});
		...
		cls : "x-btn-text-icon",
		handler:function(){
			Ext.getCmp("poleNameSub_zz").setValue("");
			Ext.getCmp("poleCode_zz").setValue("");
			Ext.getCmp("poleAddress_zz").setValue("");
			var store = Ext.getCmp("poleGrid_zz").getStore();
			store.load({
				params : {
					start : 0,
