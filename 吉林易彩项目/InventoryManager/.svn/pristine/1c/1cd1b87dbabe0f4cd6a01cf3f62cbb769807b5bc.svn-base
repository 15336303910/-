...
// 一页显示多少条数据
var limit = null;
com.increase.cen.document.dataImport = Ext.extend(Ext.form.FormPanel, {
	...
	initComponent : function(config) {
		...
		this.one = {
			...
			style : '',
			html : '<img border="0" src="imgs/plwh/s.gif" width="28" height="21"> 第一步 <a target="_self" href="export.jsp?filename=docequttmpl">下载模板</a>并按模板格式整理数据。',
		...
		com.increase.cen.document.dataImport.superclass.initComponent.call(this);
	},
	// 保存按钮事件
	btnSubmit : function() {
		// 提交验证
		...
		Ext.MessageBox.show({
			...
			fn : function(btn) {
				if (btn == 'yes') {// 如果选择确定，则进行修改
					...
					Ext.getCmp("dataImport").getForm().doAction("submit", {
						url : 'document!importData.action',
						method : 'post',
						timeout: 1000*60*5,
						...
						success : function(form, action) {
							...
							Ext.getCmp("dataImport").getForm().reset();// 重置表单
							if(Ext.getCmp('dataImportWindow')!=null){
								Ext.getCmp('dataImportWindow').hide();// 窗口隐藏
							}
...
Ext.reg("dataImport", com.increase.cen.document.dataImport);
