...
@Controller("com.function.nettool.plugin.LocationTransfer")
@RequestMapping(value="/locationTransfer")
public class LocationTransfer{
	 ...
	 * 
	 * */
	@RequestMapping("/locationtransfer.ilf")
	public void locationtransfer(@RequestParam String params,HttpServletRequest request,HttpServletResponse response)throws Exception{
		JSONObject resultObject = JSONObject.fromObject("{success:true}");
		try{
			final JSONObject paramObject = JSONObject.fromObject(params);	
			String addressName = java.net.URLEncoder.encode(paramObject.getString("ADDRESS_VALUE"),"UTF-8");
			String encodeUrl = "http://restapi.amap.com/v3/geocode/geo?key="+enterpriseKey+"&address="+addressName;
			/*
			 * 	根据链接和参数打开连接
			 * 
			 * */
			...
			if(Integer.parseInt(resultObj.get("status").toString())==1){
				...
				resultObject.put("EXPORT_X",resultObj.getJSONArray("geocodes").getJSONObject(0).getString("location").split(",")[0]);
				resultObject.put("EXPORT_Y",resultObj.getJSONArray("geocodes").getJSONObject(0).getString("location").split(",")[1]);
			}else{
	...
	public void downloadTemplate(HttpServletRequest request,HttpServletResponse response)throws Exception{
		String strDirPath = request.getSession().getServletContext().getRealPath("/")+"templates/LocationTransfer.xlsx";
			...
			if(!fileToUpload.isEmpty()){
				...
				while(sheetCar.hasNext()){
		            ...
		            if(rowNumber>=3){
		            	...
		            	jdbcTemplate.execute("INSERT INTO NET_PLUGIN_JOB(ID,JOB_TYPE,START_DATE) VALUES("+jobCode+",'LOCATION_XY',SYSDATE)");
		            	/*
		            	 * 	保存数据
		            	 * 
		            	 * */
						...
						sql+="	ID,BELONG_JOB,ADDRESS_INPUT,TRANSFER_RESULT,X_OUTPUT,Y_OUTPUT";
						sql+=")values(SEQ_NET_JOB_DATA.NEXTVAL,"+jobCode+",?,?,?,?)";					
						PreparedStatement pStatement = connection.prepareStatement(sql);
		            	for(int j=3;j<=rowNumber;j++){
			                XSSFRow currentRow = sheet.getRow(j);
			                if(currentRow!=null){
			                	XSSFCell addressValueCell = currentRow.getCell(1);
			                	if(addressValueCell!=null && !"".equals(addressValueCell.getStringCellValue())){
			                		 ...
			                		 * 
			                		 * */
			                		String addressName = java.net.URLEncoder.encode(addressValueCell.getStringCellValue(),"UTF-8");
			            			String encodeUrl = "http://restapi.amap.com/v3/geocode/geo?key="+enterpriseKey+"&address="+addressName;
			                		/*
			                		 * 	执行转化
			                		 * 
			            			...
			            			if(Integer.parseInt(resultObj.get("status").toString())==1){
			            				...
			            				X_OUTPUT = resultObj.getJSONArray("geocodes").getJSONObject(0).getString("location").split(",")[0];
			            				Y_OUTPUT = resultObj.getJSONArray("geocodes").getJSONObject(0).getString("location").split(",")[1];
			            			}else{
			                		 ...
			                		 * 
			                		 * */
			            			pStatement.setString(1,addressValueCell.getStringCellValue());
									pStatement.setString(2,TRANSFER_RESULT);
									pStatement.setString(3,X_OUTPUT);
									pStatement.setString(4,Y_OUTPUT);
									pStatement.addBatch();
									/*
									 * 	写入文件
									 * 
