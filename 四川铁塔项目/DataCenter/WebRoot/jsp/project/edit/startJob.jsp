		...
		<script type="text/javascript">
			...
			function doExport(){
				try{ 
					...
					exportForm.attr("action",contextPath+"/plugins/resCheck/dataCheckController/exportDataByCity.ilf?flowId="+$("#HIDDEN_OF_FLOW_ID").val());
					...
					success:function(textStatus){
						if(textStatus["SUCCESS"]){
							...
							if(!textStatus["IS_UPLOADED_IMAGE"]){
								...
								tableHtml+="		<center>";
								tableHtml+="			<div onclick='javascript:openUploadWindow();' style='width:98%;height:175px;border:dotted 1px #A3D0E3;border-radius:10px;cursor:pointer;'>";
								tableHtml+="				<div style='margin-top:55px;'>";
										...
										success:function(pageResult){
											if(pageResult["SUCCESS"]){
												...
												for(var b=0;b<items.length;b++){
													...
													tableHtml+="				<div style='float:left;height:175px;width:5%;'>";
													tableHtml+="					<input type='checkbox' value='"+appObject["ID"]+"' name='imageCheckbox' onchange='javascript:uniqueCheckbox(this.checked,this.name,this.value);' style='margin-top:75px;margin-left:3px;'></input>";
													tableHtml+="				</div>";
													tableHtml+="				<div style='float:left;height:175px;width:95%;'>";
			...
			function submitJob(){
				var flowNode = getNodeOfFlowByOrder(2);
				if($("#TemplateNamees").val()==""){
					window.wxc.xcConfirm("请先上传[项目交付单 ]文件.",window.wxc.xcConfirm.typeEnum.info);
					return;
				}
				if($("#IsUploadedImage").val()=="N"){
					window.wxc.xcConfirm("请先上传[项目各项完工照片  ]文件.",window.wxc.xcConfirm.typeEnum.info);
					return;
				}
				var checkedUser = getCheckedValue("userCheckbox");
				if(checkedUser==null){
				...
				window.wxc.xcConfirm("是否确定要传递到下一环节执行人？","custom",{
					...
					onOk:function(){
						$.ajax({
							url:"${pageContext.request.contextPath}/projectCheckinAction/startNewJob.ilf",
							async:false,
							type:"POST",
							data:"projectId="+$("#hiddenOfProjectId").val()+"&nextOperateUser="+checkedUser+"&flowNode="+JSON.stringify(flowNode),
							dataType:"json",
							timeout:10000, 
							success:function(textStatus){
								if(textStatus["SUCCESS"]){
									window.wxc.xcConfirm("任务已成功发起.","custom",{
										title:"提示",
										btn:parseInt("0011",2),
										onOk:function(){
											window.parent.closeAssignedTab("发起项目外验交付任务");
										}
									});
								}else{
  	...
  	<body style="width:99.8%;border:solid 1px #FFF;" id="bodyHeight">
  		...
  		<div style="width:99.8%;height:125px;" id="topPanel">
  			<div class="panel panel-default" style="margin-top:-20px;height:115px;border:solid 0px #FFF;">
				<div class="panel-heading" id="panelHeading" style="border:solid 1px #A3D0E3;border-bottom:0px;">
					<span class="panel-label"></span>Step-1.请上传项目交付单
				</div>
						...
						<div class="div_scroll">
							<div style="height:auto;width:auto">
								<div style="float:left;">
									...
									<input type="file" id="TemplateUpload" name="TemplateUpload" style="cursor:pointer;width:450px;height:30px;border:solid 1px #A3D0E3;"></input>
								</div>
  		...
  		<div style="width:99.8%;height:400px;" id="centerPanel">
  			<div class="panel panel-default" style="margin-top:-20px;height:400px;border:solid 0px #FFF;" id="subOfCenterPanel">
				<div class="panel-heading" id="panelHeading" style="border:solid 1px #A3D0E3;border-bottom:0px;">
					<span class="panel-label"></span>Step-2.请上传项目各项完工照片
  		...
  		<div style="width:99.8%;height:300px;">
  			<div class="panel panel-default" style="height:300px;border:solid 0px #FFF;">
				<div class="panel-heading" id="panelHeading" style="border:solid 1px #A3D0E3;border-bottom:0px;">
					<span class="panel-label"></span>Step3.选择下一环节审批人.
					<form class="form-search pull-right">	
						<div style="float:right;">
							...
							<a class="btn btn-success" style="cursor:pointer" onclick="javascript:submitJob();">
								<i class="icon-ok icon-white"></i>发起任务
							</a>
						</div>	
					</form>
				</div>
