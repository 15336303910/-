	...
	<head>
		...
		<script type="text/javascript">
			...
			$(document).ready(function(){
				...
				/*首先向后台获取身份（省级/地市）*/
				$.ajax({
					url:"${pageContext.request.contextPath}/riskQuestionAmountAction/findRiskQuestionFirst.ilf",
					async:true,
					type:"POST",
					...
					success:function(riskData){
						if(riskData["success"]){
							...
							oTable=$('#totalData').dataTable({
								...
								"sAjaxSource":"${pageContext.request.contextPath}/riskQuestionAmountAction/findTableData.ilf"
							});
						}
					},
					error:function(){}
			...
			function searchData(){
				...
				/*查询地级市问题总数*/
				$.ajax({
					url:"${pageContext.request.contextPath}/riskQuestionAmountAction/findAfterSearchNumData.ilf",
					async:true,
					type:"POST",
					dataType:"json",
			...
			function showDetail(riskType,city,mouth,account){
				...
				var url = "jsp/firstIndex/riskControlOnline/riskQuestionAmountDetail.jsp?riskType="+riskType+"&city="+city+"&mouth="+mouth+"&account="+account;
				url = encodeURI(url);
				window.parent.parent.turnToJsp("风险问题详情",url);
			}
		</script>
		<style type="text/css">
			.table th,.table td{
		...
		<div class="panel panel-primary"  id="mainPanel">
			<div id="panelHeading" style="width:100%;height:50px;background-color:#337ab7;display:table;padding-left:10px">
				<div style="background-color:#337ab7;display:table-cell;vertical-align:middle;padding-left:10px">
					<font color="white" size="5">在线风控--风险点问题总数</font>
				</div>
			</div>
				...
				<div style="width:100%;height:50px">
			 	...
			 	<div class="col-xs-12 col-sm-12 col-md-12"
					...
					<div style="width:100%;height:40px;">
						<font size="4">风险点问题数量:</font>
						<!--<button type="button" class="btn btn-danger btn-sm" style="width:70px;"><span id="question_amount">-</span></button>-->
					    <!-- 此处使用按钮不妥，看起来很像可钻取功能 -->
					    <font size="4" color="red"><span id="question_amount">-</span></font>
					</div>
