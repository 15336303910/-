		...
		<script type="text/javascript">
			...
			$(document).ready(function(){
				...
				oTable = $("#dataTable").dataTable({
					...
					"sAjaxSource":"${pageContext.request.contextPath}/transferXYAction/findItems.ilf"
				});
			...
			function downloadTemplate(){
				window.wxc.xcConfirm("是否确认下载模板文件？.","custom",{
					...
					onOk:function(){
					    ...
					    exportForm.attr("action","${pageContext.request.contextPath}/transferXYAction/downloadTemplate.ilf");
			...
			function doTransfer(){
				...
				if(editObject["xPoint"]==""){
					window.wxc.xcConfirm("请输入原始经度.",window.wxc.xcConfirm.typeEnum.info);
					return;
				}
				if(editObject["yPoint"]==""){
					window.wxc.xcConfirm("请输入原始纬度.",window.wxc.xcConfirm.typeEnum.info);
					return;
				}
				$.ajax({
					url:"${pageContext.request.contextPath}/transferXYAction/xytransfer.ilf",
					async:false,
					type:"POST",
			...
			function doUpload(){
				...
				if(uploadFileName.indexOf(".xlsx")==-1){
					...
					$.ajaxFileUpload({
						url:"${pageContext.request.contextPath}/transferXYAction/uploadDataFile.ilf",
						secureuri:false,
  	...
  	<body style="width:100%;">
		...
		<div class="panel panel-default" style="margin-top:1px;height:780px;border:solid 1px #FFF;" class="models" id="mainPanel">
			<div class="panel-heading">
				<span class="panel-label"></span>高德坐标系 >> 将非高德坐标转化为高德坐标系坐标
										...
										<table cellpadding="0" cellspacing="0" border="0" id="dataTable" style="border:0px;">
											<thead>
												<tr>
													...
													<th><font color="red">输入类型</font></th>
													<th>转化结果</th>
			...
			<div class="panel-body h340" style="border:0px;display:none;" id="singleTransfe">
				<center>
					<div style="width:60%;height:395px;overflow:auto;border:dotted 2px #A3D0E3;border-radius:10px;margin-top:30px;">
						<center>
							<table cellpadding="0" cellspacing="0" style="width:90%;font-size:14px;margin-top:20px;">
								<tr height="50px;">
									<td style="width:20%;text-align:right;">原始经度：<font color="red" style="margin-right:3px;">*</font></td>
								...
								<tr height="50px;">
									<td style="width:20%;text-align:right;">原始纬度：<font color="red" style="margin-right:3px;">*</font></td>
									<td style="width:80%;" colspan="3">
								...
								<tr height="50px;">
									<td style="width:20%;text-align:right;">输入位置类型：<font color="red" style="margin-right:3px;">*</font></td>
									<td style="width:80%;" colspan="3">
										<select id="XY_TYPE" style="width:80%;height:35px;border:solid 1px #A3D0E3;border-radius:6px;margin-top:7px;">
											<option value="gps">GPS</option>
											<option value="mapbar">MAPBAR</option>
											<option value="baidu">百度地图坐标</option>
										</select>
									</td>
								</tr>
								<tr height="50px;">
