		...
		<script type="text/javascript">
			...
			$(document).ready(function(){
				...
				oTable = $("#dataTable").dataTable({
					...
					"sAjaxSource":"${pageContext.request.contextPath}/projectCheckinAction/findItems.ilf"
				});
			});
			function searchData(){
			...
			function startCheckJob(){
				...
				if(checkedValue!=null){
					...
					if(projectMap["CURRENT_NODE"]==null){
						window.parent.turnToJsp("发起项目外验交付任务","jsp/project/edit/startJob.jsp?prjId="+checkedValue);	
					}else{
						if(projectMap["CURRENT_NODE"].indexOf("驳回")!=-1){
							window.parent.turnToJsp("发起项目外验交付任务","jsp/project/edit/startJob.jsp?prjId="+checkedValue);	
						}else{
							window.wxc.xcConfirm("此项目已发起外验.",window.wxc.xcConfirm.typeEnum.info);	
						}
					}
				}else{
  	...
  	<body style="width:99.8%;border:solid 1px #FFF;" id="bodyHeight">
		...
		<div class="panel panel-default" style="margin-top:-20px;height:485px;border:solid 0px #FFF;" id="mainPanel">
			<div class="panel-heading" id="panelHeading" style="border:solid 1px #A3D0E3;border-bottom:0px;">
				...
				<span class="panel-label"></span>项目管理 >> 外验交付任务发起
				<form class="form-search pull-right">	
					<div style="float:right;">
						...
						<input id="PRO_NAME_INPUT" type="text" placeholder="请输入项目名称" style="cursor:pointer;width:200px;height:29px;font-size:12px;border:solid 1px #A3D0E3;border-top-left-radius:5px;border-bottom-left-radius:5px;"></input>
						<a class="btn btn-primary" style="cursor:pointer" onclick="javascript:searchData();">
							<i class="icon-search icon-white"></i>执行查询 
						</a>
						<a class="btn btn-success" style="cursor:pointer" onclick="javascript:startCheckJob();">
							<i class="icon-flag icon-white"></i>发起外验交付 
						</a>
